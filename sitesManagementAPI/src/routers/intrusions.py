...
def create_intrusion(intrusion: schemas.IntrusionCreate, user_id: str, property_id: int, db: Session = Depends(get_db), \
...
def read_intrusions_by_user(user_id: str, db: Session = Depends(get_db), user: OIDCUser = Depends(idp.get_current_user(required_roles=['g5-end-users']))):
    try:
        idp.get_user(user_id=user_id, query=None)
    except ExpiredSignatureError:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Signature expired")
...
def update_intrusion(intrusion_id: int, updated_intrusion: schemas.IntrusionBase, new_property_id: int | None = None, new_user_id: str | None = None, db: Session = Depends(get_db), \
    ...
    if new_user_id:
        try:
            idp.get_user(user_id=new_user_id, query=None)
        except ExpiredSignatureError:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Signature expired")
        except: 
