...
def get_user_videos(db: Session, user_id: int):
    ...
    query = db.query(models.VideoUsers).filter(models.VideoUsers.user_id == user_id).all()
    print(query) #should return a list of videos
    return query
...
def save_on_s3_bucket(access_key_id, secret_access_key, region_name, bucket_name, filename, file):
    ...
    try:
        client.upload_file(Bucket=bucket_name, Key=filename, Filename=".videos/" + filename)
        return True
...
def get_from_s3_bucket(access_key_id, secret_access_key, region_name, bucket_name, filename):
    ...
    try:
        client.download_file(Bucket=bucket_name, Key=filename, Filename="./videos/" + filename)
